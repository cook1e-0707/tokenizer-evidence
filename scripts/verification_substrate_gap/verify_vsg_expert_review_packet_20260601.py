#!/usr/bin/env python3
"""Verify the refreshed 2026-06-01 VSG expert review packet."""
...
DATE_TAG = "20260601"
DEFAULT_PACKET_DIR = ROOT / "results" / "verification_substrate_gap" / f"expert_review_packet_{DATE_TAG}"
DEFAULT_ZIP_PATH = ROOT / "results" / "verification_substrate_gap" / f"vsg_expert_review_packet_{DATE_TAG}.zip"
DEFAULT_ZIP_SHA_PATH = ROOT / "results" / "verification_substrate_gap" / f"vsg_expert_review_packet_{DATE_TAG}.zip.sha256"
DEFAULT_EXTERNAL_README = ROOT / "results" / "verification_substrate_gap" / f"vsg_expert_review_packet_{DATE_TAG}_README.txt"

REQUIRED_FILES = {
    f"EXPERT_REVIEW_SCOPE_{DATE_TAG}.md",
    f"OBJECTIVE_FACTS_{DATE_TAG}.md",
    f"HARDENING_STATUS_{DATE_TAG}.md",
    f"README_FOR_EXPERT_REVIEW_{DATE_TAG}.md",
    f"manuscript/VSG_manuscript_snapshot_{DATE_TAG}.pdf",
    "manuscript_source/main.tex",
    "manuscript_source/section_01_introduction.tex",
    "manuscript_source/section_02_related_work.tex",
    ...
    "manuscript_source/figures/figure_1_verification_substrate_map.png",
    "manuscript_source/figures/figure_4_public_predicate_attack_ladder.png",
    ...
    "validation/hardening_summary.json",
...
def verify(args: argparse.Namespace) -> dict:
    ...
    if manifest.get("status") != "PASS_PACKET_ASSEMBLED_ARTIFACT_ONLY_20260601_HARDENING_INCLUDED":
    ...
    if not str(git_snapshot.get("manuscript_repository_head", "")).startswith("c10b3f1"):
    ...
    if git_snapshot.get("overleaf_push_attempted_in_this_packet") is not False:
        fail("overleaf_push_attempted_in_this_packet is not false", failures)

    hardening = load_json(packet_dir / "validation" / "hardening_summary.json")
    if hardening.get("public_text_stronger_baseline", {}).get("codeword_recovered_blocks_total") != 0:
        fail(f"unexpected public text codeword recovery: {hardening}", failures)
    if hardening.get("attack_naturalness_proxy", {}).get("proxy_pass_rows") != 0:
        fail(f"unexpected proxy-readable attack rows: {hardening}", failures)
    if hardening.get("reproducibility_release_inventory", {}).get("release_ready_without_review") is not False:
        fail(f"unexpected release-ready flag: {hardening}", failures)
    if hardening.get("ownership_decision_rule_audit", {}).get("supported_public_text_row_count") != 0:
        fail(f"unexpected supported public-text ownership rows: {hardening}", failures)
    if hardening.get("manuscript_figure_quality_audit", {}).get("failed_figure_count") != 0:
        fail(f"unexpected failed figure count: {hardening}", failures)
    if hardening.get("manuscript_figure_quality_audit", {}).get("failed_data_check_count") != 0:
        fail(f"unexpected failed data checks: {hardening}", failures)
