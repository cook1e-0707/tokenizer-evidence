...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Prepare G3a-v2 block-scale manifests.")
    parser.add_argument(
        "--package-config",
        default="configs/reporting/g3a_block_scale_v2.yaml",
        help="YAML package config for G3a-v2.",
    )
    parser.add_argument("--phase", choices=["pilot", "final"], default="final")
    parser.add_argument(
        "--output",
        default="results/processed/paper_stats/g3a_v2_package_dry_run.json",
        help="JSON output path for the dry-run package summary.",
    )
    parser.add_argument(
        "--train-manifest-out",
        default="manifests/g3a_v2_qwen7b_block_scale/train_manifest.json",
        help="Output path for the train manifest.",
    )
    parser.add_argument(
        "--eval-manifest-out",
        default="manifests/g3a_v2_qwen7b_block_scale/eval_manifest.json",
        help="Output path for the eval manifest.",
    )
    parser.add_argument(
        "--output-root-base",
        help="Optional base directory for G3a-v2 case roots. Defaults to EXP_SCRATCH/g3a_block_scale_v2.",
    )
    parser.add_argument(
        "--environment-setup",
...
def _resolve_output_root_base(package_config: dict[str, Any], explicit: str | None) -> str:
    ...
    print(
        ...
        "new_case_root_prefix. Set EXP_SCRATCH on Chimera so G3a-v2 large outputs stay off home.",
        file=sys.stderr,
    )
    return prefix
...
def _variant_name(phase: str, case: dict[str, Any]) -> str:
    ...
    return f"g3a-v2-qwen7b-block-scale-{phase}-{case['variant_slug']}{hp}"
                ...
                cases.append(
                    {
                        ...
                        "case_root": _case_root(
                            output_root_base,
                            "final",
                            str(variant.get("slug", str(variant["id"]).lower())),
                            str(payload),
                            int(seed),
                        ),
                        "hp_id": "",
...
def _pilot_cases(package_config: dict[str, Any], output_root_base: str) -> list[dict[str, Any]]:
...
def _build_train_entry(
    *,
    ...
    manifest_id = (
        f"g3a-v2-{phase}-train-{case['hp_id'] + '-' if case.get('hp_id') else ''}"
        f"{case['variant_slug']}-{str(case['payload']).lower()}-s{case['seed']}"
    )
    return _entry_with_identity(
        manifest.entries[0],
        manifest_id=manifest_id,
        manifest_name=f"g3a_v2_qwen7b_block_scale_{phase}_train",
    )
...
def _build_eval_entry(
    *,
    ...
    manifest_id = (
        f"g3a-v2-{phase}-eval-{case['hp_id'] + '-' if case.get('hp_id') else ''}"
        f"{case['variant_slug']}-{str(case['payload']).lower()}-s{case['seed']}"
    )
    return _entry_with_identity(
        manifest.entries[0],
        manifest_id=manifest_id,
        manifest_name=f"g3a_v2_qwen7b_block_scale_{phase}_eval",
    )
...
def main() -> int:
    ...
    cases = (
        _pilot_cases(package_config, output_root_base)
        if args.phase == "pilot"
    ...
    _save_manifest(
        train_config_path,
        f"g3a_v2_qwen7b_block_scale_{args.phase}_train",
        train_entries,
        train_manifest_path,
    )
    _save_manifest(
        eval_config_path,
        f"g3a_v2_qwen7b_block_scale_{args.phase}_eval",
        eval_entries,
        eval_manifest_path,
    )

    payload = {
        "schema_name": "g3a_v2_package_dry_run",
        "schema_version": 1,
    ...
    print(f"wrote G3a-v2 {args.phase} dry-run summary to {output_path}")
