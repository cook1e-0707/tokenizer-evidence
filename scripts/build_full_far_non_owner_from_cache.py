...
from scripts.run_full_far_payload_claim_benchmark import (
    ...
    _load_yaml,
    _non_owner_probe_ids_for_budget,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Stage 2 for full FAR non-owner-probe null: expand probe-level "
            "base-Qwen cache into completed non-owner FAR row shards without GPU inference."
        )
    )
...
def _load_tokenizer(cfg: dict[str, Any], model_name: str) -> Any:
    ...
    if tokenizer is None:
        raise RuntimeError(
            "Could not load tokenizer for non-owner cache expansion. Tried:\n"
            + "\n".join(errors)
...
def _cache_paths(cache_dir: Path) -> list[Path]:
    return sorted(cache_dir.glob("full_far_payload_claim_non_owner_probe_cache_shard_*_of_*.csv"))


def _row_shard_paths(row_shard_dir: Path, shard_index: int, shard_count: int) -> dict[str, Path]:
    stem = f"full_far_payload_claim_non_owner_probes_shard_{shard_index:03d}_of_{shard_count:03d}"
    return {
...
def _read_cache_rows(paths: list[Path]) -> dict[str, dict[str, str]]:
    by_probe_id: dict[str, dict[str, str]] = {}
        ...
        with path.open("r", encoding="utf-8", newline="") as handle:
            for row in csv.DictReader(handle):
                if str(row.get("status", "")) != "completed_probe_cache":
                    continue
                probe_id = str(row.get("non_owner_probe_id", ""))
                if not probe_id.startswith("non_owner_probe_"):
                    continue
                if probe_id in by_probe_id:
                    duplicates.add(probe_id)
                enriched = dict(row)
                enriched["cache_source"] = str(path)
                by_probe_id[probe_id] = enriched
    if duplicates:
        raise RuntimeError(f"Duplicate non-owner probe cache rows: {sorted(duplicates)[:10]}")
    return by_probe_id


def _validate_cache_coverage(cfg: dict[str, Any], cache_by_probe_id: dict[str, dict[str, str]]) -> None:
    probe_count = int((cfg.get("null_protocol") or {}).get("non_owner_probe_count", 0))
    missing = [
        f"non_owner_probe_{index:04d}"
        for index in range(probe_count)
        if f"non_owner_probe_{index:04d}" not in cache_by_probe_id
    ]
    if missing:
        raise RuntimeError(
            f"Non-owner probe cache is incomplete: missing {len(missing)} probes; "
            f"first_missing={missing[:10]}"
        )


def _required_non_owner_rows_for_shard(
    ...
    *,
    ...
    for row in rows:
        if str(row.get("evaluation_type", "")) != "non_owner_probe_null":
            continue
        if str(row.get("null_model_id", "")) != "base_qwen":
            continue
        probe_index = _organic_index(str(row.get("organic_prompt_id", "non_owner_probe_0000")))
        if start <= probe_index < end:
            selected.append(row)
    if not selected:
        raise RuntimeError(f"No non-owner rows selected for shard {shard_index}/{shard_count}.")
    return selected


def _non_owner_seed_for_row(cfg: dict[str, Any], row: dict[str, Any]) -> int:
    seeds = _seeds(cfg)
    if not seeds:
        return 0
    return seeds[_organic_index(str(row.get("organic_prompt_id", "non_owner_probe_0000"))) % len(seeds)]
...
def _ours_from_cache_result(
    *,
    ...
    cfg: dict[str, Any],
    row: dict[str, Any],
    cache_by_probe_id: dict[str, dict[str, str]],
    tokenizer: Any,
    model_name: str,
    context: dict[str, Any],
) -> dict[str, Any]:
    claim_payload = str(row.get("claim_payload", ""))
    non_owner_probe_id = str(row.get("organic_prompt_id", "non_owner_probe_0000"))
    budget = max(1, _safe_int(row.get("query_budget"), 1))
    probe_ids = _non_owner_probe_ids_for_budget(
    ...
    for probe_id in probe_ids:
        cache_row = cache_by_probe_id[probe_id]
    ...
    accept_count = sum(1 for item in probe_results if item["claim_accept"])
    return {
        "claim_accept": accept_count > 0,
        "ownership_score": accept_count / len(probe_results) if probe_results else 0.0,
        "non_owner_probe_id": non_owner_probe_id,
        "non_owner_probe_ids": probe_ids,
        "claim_payload": claim_payload,
        "model_name": model_name,
        "fresh_query_count": len(probe_ids),
        "accept_count": accept_count,
        "probe_results": probe_results,
        "decision_rule": "accept_if_any_cached_non_owner_probe_decodes_claim_payload",
    }
...
def _perinucleus_from_cache_result(
    *,
    ...
    budget = max(1, _safe_int(row.get("query_budget"), 1))
    non_owner_probe_id = str(row.get("organic_prompt_id", "non_owner_probe_0000"))
    selected_seed = _non_owner_seed_for_row(cfg, row)
    ...
    exact_count = 0
    for probe_id, selected in zip(probe_ids, selected_rows, strict=True):
    ...
    return {
        ...
        "decision_rule": "all_selected_fingerprint_responses_match_cached_non_owner_probe_window",
    }
    ...
    *,
    ...
    if method_id == OURS_METHOD_ID:
        result = _ours_from_cache_result(
            ...
            cfg=cfg,
            row=row,
            cache_by_probe_id=cache_by_probe_id,
        ...
        completed = _set_fresh_completed(
            ...
            status=FRESH_NON_OWNER_NULL_STATUS,
            execution_backend="fresh_non_owner_probe_null_from_cache_v1",
            execution_scope="ours_base_qwen_non_owner_probe_cache_null",
            source_status="fresh_base_qwen_non_owner_probe_cache_completed",
            notes_suffix="fresh base-Qwen non-owner probe null from probe cache",
            details=result,
        )
        ...
        completed = _set_fresh_completed(
            ...
            status=FRESH_NON_OWNER_NULL_STATUS,
            execution_backend="fresh_non_owner_probe_null_from_cache_v1",
            execution_scope="perinucleus_base_qwen_non_owner_probe_cache_null",
            source_status="fresh_base_qwen_non_owner_probe_cache_completed",
            notes_suffix="fresh base-Qwen non-owner probe null from probe cache",
            details=result,
        )
    else:
        raise RuntimeError(f"Unsupported non-owner method_id={method_id}")
    first_probe = str(row.get("organic_prompt_id", "non_owner_probe_0000"))
    completed["source_artifact"] = str(cache_by_probe_id[first_probe].get("cache_source", ""))
    return completed
...
def main() -> int:
    ...
    if not cache_paths:
        raise FileNotFoundError(f"No non-owner probe cache CSVs found in {cache_dir}.")
    cache_by_probe_id = _read_cache_rows(cache_paths)
    _validate_cache_coverage(cfg, cache_by_probe_id)
    ...
    for shard_index in shard_indices:
        shard_plan_rows = _required_non_owner_rows_for_shard(
            plan_rows,
            cfg=cfg,
        ...
        summary["shard"] = {
            "fresh_null_mode": "non-owner-probes",
            "shard_index": shard_index,
            "shard_count": shard_count,
            ...
            "note": "Built from probe cache; aggregate all shards before interpreting FAR.",
        }
        summary["probe_cache"] = {
            "cache_dir": str(cache_dir),
            "cache_csv_count": len(cache_paths),
            "cache_probe_count": len(cache_by_probe_id),
        ...
        print(
            json.dumps(
                {
                    "event": "non_owner_row_shard_written",
                    "shard_index": shard_index,
                    "shard_count": shard_count,
                    "completed_case_count": len(completed_rows),
                    "table": str(paths["table"]),
    ...
    print(
        json.dumps(
            {
                "status": "non_owner_row_shards_built_from_cache",
                "cache_dir": str(cache_dir),
