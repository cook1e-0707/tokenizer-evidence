...
SBATCH_ARGS=(
  ...
  --export=HOME,REPO_HOME="$REPO_HOME",SWEEP_CONFIG="$SWEEP_CONFIG",SCRATCH_ROOT="$SCRATCH_ROOT",VENV_PATH="$VENV_PATH"
)
...
sbatch "${SBATCH_ARGS[@]}" "$REPO_HOME/scripts/slurm_perinucleus_qwen_capacity_sweep.sbatch"
