...
DEFAULT_CONFIG = ROOT / "configs/natural_evidence_v2/r4_after_870987_same_family_raw_null_v7_tokenizer_preflight_route.yaml"
...
EXPECTED_ROWS_PER_TOKENIZER = 15360


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate R4 same-family raw-null v7 tokenizer preflight route.")
...
def main() -> int:
    ...
    if cfg.get("schema_name") != "natural_evidence_v2_r4_after_870987_same_family_raw_null_v7_tokenizer_preflight_route_v1":
        errors.append("schema_name mismatch")
    if cfg.get("route_id") != "r4_after_870987_same_family_raw_null_v7_tokenizer_preflight_v1":
    ...
    row_manifest = read_json(root_path(source.get("v7_row_bank_manifest", ""), errors, "v7 row bank manifest"))
    ...
    if failed_summary.get("status") != "FAIL_R4_AFTER_870987_SAME_FAMILY_RAW_NULL_GENERATION_GATE":
        errors.append("877751 aggregate summary must remain failed")
    ...
    if rows_path.exists() and sum(1 for line in rows_path.open("r", encoding="utf-8") if line.strip()) != EXPECTED_ROWS_PER_TOKENIZER:
        errors.append(f"v7 row bank must contain {EXPECTED_ROWS_PER_TOKENIZER} rows")
    ...
    for fragment in (
        "ROUTE_CONFIG=configs/natural_evidence_v2/r4_after_870987_same_family_raw_null_v7_tokenizer_preflight_route.yaml",
        ...
        "MAX_ROWS=15360",
        "ROUTE_VALIDATOR=scripts/natural_evidence_v2/validate_r4_after_870987_same_family_raw_null_v7_tokenizer_route.py",
    ...
    if entry is None:
        ...
        for fragment in (
            "r4_after_870987_same_family_raw_null_v7_tokenizer_preflight_route.yaml",
            "r4_after_870987_same_family_raw_null_v7_row_bank_plan_20260526/row_allocation_rows.jsonl",
            "validate_r4_after_870987_same_family_raw_null_v7_tokenizer_route.py",
        ):
            if fragment not in entry_command:
                errors.append(f"allowlist command_pattern missing fragment: {fragment}")

    status = (
        "PASS_R4_AFTER_870987_SAME_FAMILY_RAW_NULL_V7_TOKENIZER_ROUTE_PLAN_ONLY_NO_SUBMIT"
        if not errors
        else "FAIL_R4_AFTER_870987_SAME_FAMILY_RAW_NULL_V7_TOKENIZER_ROUTE_PLAN_ONLY_NO_SUBMIT"
    )
    ...
    summary = {
        "schema_name": "natural_evidence_v2_r4_after_870987_same_family_raw_null_v7_tokenizer_route_validation_v1",
        "status": status,
        "errors": errors,
    ...
    write_text_new(
        ...
        "# R4 Same-Family Raw-Null V7 Tokenizer Route Validation\n\n"
        f"Status: `{status}`\n\n"
        f"Errors: {len(errors)}\n\n"
