...
DEFAULT_CONFIG = ROOT / "configs/natural_evidence_v2/r4_after_877840_same_family_raw_null_v8_tokenizer_preflight_route.yaml"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate R4 after-877840 same-family raw-null v8 tokenizer route.")
...
def main() -> int:
    ...
    if cfg.get("schema_name") != "natural_evidence_v2_r4_after_877840_same_family_raw_null_v8_tokenizer_preflight_route_v1":
        errors.append("schema_name mismatch")
    if cfg.get("route_id") != "r4_after_877840_same_family_raw_null_v8_tokenizer_preflight_v1":
        errors.append("route_id mismatch")

    source = mapping(cfg.get("source_artifacts"), errors, "source_artifacts")
    pass_summary = read_json(root_path(source.get("capacity_limited_generation_summary_877840", ""), errors, "877840 pass summary"))
    feasibility = read_json(root_path(source.get("current_source_feasibility", ""), errors, "current source feasibility"))
    ...
    if pass_summary.get("status") != "PASS_R4_AFTER_870987_SAME_FAMILY_RAW_NULL_GENERATION_GATE":
        errors.append("877840 aggregate summary must be the reviewed capacity-limited pass")
    if not bool(pass_summary.get("same_family_raw_null_gate_pass")):
        errors.append("877840 summary must have same_family_raw_null_gate_pass=true")
    if feasibility.get("status") != "BLOCK_R4_AFTER_877840_FULL_SAME_FAMILY_RAW_NULL_CURRENT_SOURCE_INSUFFICIENT_NO_SUBMIT":
        errors.append("current-source feasibility blocker must be recorded")
    if feasibility.get("full_64_shard_feasible_from_current_source") is not False:
        errors.append("feasibility must show current source is not full-64 ready")
    ...
    if int(row_manifest.get("selected_prompt_count", -1)) != 4096:
        errors.append("v8 row bank must have 4096 selected prompts")
    if int(row_manifest.get("selected_domain_count", -1)) < 32:
        errors.append("v8 row bank selected_domain_count must be >=32")
    if rows_path.exists() and sum(1 for line in rows_path.open("r", encoding="utf-8") if line.strip()) != EXPECTED_ROWS_PER_TOKENIZER:
        errors.append(f"v8 row bank must contain {EXPECTED_ROWS_PER_TOKENIZER} rows")
    ...
    for fragment in (
        "ROUTE_CONFIG=configs/natural_evidence_v2/r4_after_877840_same_family_raw_null_v8_tokenizer_preflight_route.yaml",
        ...
        "MAX_ROWS=65536",
        "ROUTE_VALIDATOR=scripts/natural_evidence_v2/validate_r4_after_877840_same_family_raw_null_v8_tokenizer_route.py",
    ...
    if entry is None:
        ...
        for fragment in (
            "r4_after_877840_same_family_raw_null_v8_tokenizer_preflight_route.yaml",
            "r4_after_877840_same_family_raw_null_v8_row_bank_plan_20260526/row_allocation_rows.jsonl",
            "validate_r4_after_877840_same_family_raw_null_v8_tokenizer_route.py",
            "MAX_ROWS=65536",
        ):
    ...
    status = (
        "PASS_R4_AFTER_877840_SAME_FAMILY_RAW_NULL_V8_TOKENIZER_ROUTE_PLAN_ONLY_NO_SUBMIT"
        if not errors
        else "FAIL_R4_AFTER_877840_SAME_FAMILY_RAW_NULL_V8_TOKENIZER_ROUTE_PLAN_ONLY_NO_SUBMIT"
    )
    ...
    summary = {
        "schema_name": "natural_evidence_v2_r4_after_877840_same_family_raw_null_v8_tokenizer_route_validation_v1",
        "status": status,
        "errors": errors,
    ...
    write_text_new(
        ...
        "# R4 After-877840 Same-Family Raw-Null V8 Tokenizer Route Validation\n\n"
        f"Status: `{status}`\n\n"
        f"Errors: {len(errors)}\n\n"
