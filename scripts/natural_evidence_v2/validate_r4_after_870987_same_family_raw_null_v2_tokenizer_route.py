...
DEFAULT_CONFIG = ROOT / "configs/natural_evidence_v2/r4_after_870987_same_family_raw_null_v2_tokenizer_preflight_route.yaml"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate R4 same-family raw-null v2 tokenizer preflight route.")
...
def main() -> int:
    ...
    if cfg.get("schema_name") != "natural_evidence_v2_r4_after_870987_same_family_raw_null_v2_tokenizer_preflight_route_v1":
        errors.append("schema_name mismatch")
    if cfg.get("route_id") != "r4_after_870987_same_family_raw_null_v2_tokenizer_preflight_v1":
    ...
    row_manifest = read_json(root_path(source.get("v2_row_bank_manifest", ""), errors, "v2 row bank manifest"))
    ...
    for fragment in (
        "ROUTE_CONFIG=configs/natural_evidence_v2/r4_after_870987_same_family_raw_null_v2_tokenizer_preflight_route.yaml",
        ...
        "MAX_ROWS=65536",
        "ROUTE_VALIDATOR=scripts/natural_evidence_v2/validate_r4_after_870987_same_family_raw_null_v2_tokenizer_route.py",
    ...
    status = (
        ...
        if not errors
        else "FAIL_R4_AFTER_870987_SAME_FAMILY_RAW_NULL_V2_TOKENIZER_ROUTE_PLAN_ONLY_NO_SUBMIT"
    )
    ...
    summary = {
        "schema_name": "natural_evidence_v2_r4_after_870987_same_family_raw_null_v2_tokenizer_route_validation_v1",
        "status": status,
        "errors": errors,
        "allowlist_entry": EXPECTED_ENTRY,
        ...
        "next_allowed_action": "Run exactly one reviewed H200 same-family v2 tokenizer-only preflight array; no generation until tokenizer preflight passes and is reviewed.",
    }
    write_json_new(out / "route_validation_summary.json", summary)
    write_text_new(
        out / "route_validation_report.md",
        "# R4 Same-Family Raw-Null V2 Tokenizer Route Validation\n\n"
        f"Status: `{status}`\n\n"
        f"Errors: {len(errors)}\n\n"
