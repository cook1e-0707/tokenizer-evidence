...
DEFAULT_CONFIG = ROOT / "configs/natural_evidence_v2/r4_after_870987_same_family_raw_null_generation_v5_route.yaml"
...
EXPECTED_COMMAND_FRAGMENTS = (
    "ROUTE_CONFIG=configs/natural_evidence_v2/r4_after_870987_same_family_raw_null_generation_v5_route.yaml",
    "ROUTE_VALIDATOR=scripts/natural_evidence_v2/validate_r4_after_870987_same_family_raw_null_generation_v5_route.py",
    ...
    "ALLOCATION_ROWS=results/natural_evidence_v2/status/r4_after_870987_same_family_raw_null_v5_row_bank_plan_20260525/row_allocation_rows.jsonl",
    "TOKENIZER_REVIEW=results/natural_evidence_v2/status/r4_after_870987_same_family_raw_null_v5_tokenizer_preflight_877139_review/review_summary.json",
    "PUBLIC_RUN_SALT=r4_after_870987_same_family_raw_null_generation_v5",
    "PLAN_ONLY=0",
...
def main() -> int:
    ...
    if cfg.get("schema_name") != "natural_evidence_v2_r4_after_870987_same_family_raw_null_generation_v5_route_v1":
        errors.append("schema_name mismatch")
    if cfg.get("route_id") != "r4_after_870987_same_family_raw_null_generation_v5":
    ...
    row_manifest = read_json(root_path(source.get("v5_row_bank_rows", ""), errors, "v5 row bank rows").with_name("row_allocation_manifest.json"))
    row_validation = read_json(root_path(source.get("v5_row_bank_validation", ""), errors, "v5 row bank validation"))
    row_bank_rows = root_path(source.get("v5_row_bank_rows", ""), errors, "v5 row bank rows")
    tokenizer_review = read_json(root_path(source.get("same_family_v5_tokenizer_review", ""), errors, "same-family v5 tokenizer review"))
    ...
    if failed_review.get("status") != "FAIL_R4_AFTER_870987_SAME_FAMILY_RAW_NULL_V4_876852_FORBIDDEN_RESIDUAL_NO_ADOPT":
        errors.append("876852 failure review must remain no-adopt")
    ...
    if tokenizer_review.get("status") != "PASS_R4_AFTER_870987_SAME_FAMILY_RAW_NULL_V5_TOKENIZER_PREFLIGHT_877139":
        errors.append("same-family v5 tokenizer review 876849 must pass")
    if int(tokenizer_review.get("checked_rows_total", -1)) != 196608:
        errors.append("same-family v5 tokenizer review must check 196608 rows")
    if int(tokenizer_review.get("failed_rows_total", -1)) != 0:
        errors.append("same-family v5 tokenizer review failed rows must be 0")
    ...
    if not isinstance(repair, Mapping):
        ...
        repair = {}
    for key in ("rescue_876852", "relax_hard_forbidden_policy"):
    ...
    status = (
        ...
        if not errors
        else "FAIL_R4_AFTER_870987_SAME_FAMILY_RAW_NULL_V5_GENERATION_ROUTE_PLAN_ONLY_NO_SUBMIT"
    )
    summary = {
        "schema_name": "natural_evidence_v2_r4_after_870987_same_family_raw_null_v5_generation_route_validation_v1",
        "status": status,
        "errors": errors,
        "allowlist_entry": EXPECTED_ENTRY,
        ...
        "tokenizer_review": source.get("same_family_v5_tokenizer_review", ""),
        "slurm_allowed": False,
        "generation_started": False,
        "paper_claim_allowed": False,
        "next_allowed_action": "Run local/remote hash preflight before exactly-one H200 same-family raw-null v5 generation submission.",
    }
