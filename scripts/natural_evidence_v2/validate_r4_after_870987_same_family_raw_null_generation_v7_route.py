...
DEFAULT_CONFIG = ROOT / "configs/natural_evidence_v2/r4_after_870987_same_family_raw_null_generation_v7_route.yaml"
...
EXPECTED_TOKENIZER_STATUS = "PASS_R4_AFTER_870987_SAME_FAMILY_RAW_NULL_V7_TOKENIZER_PREFLIGHT_877837"
EXPECTED_COMMAND_FRAGMENTS = (
    "ROUTE_CONFIG=configs/natural_evidence_v2/r4_after_870987_same_family_raw_null_generation_v7_route.yaml",
    "ROUTE_VALIDATOR=scripts/natural_evidence_v2/validate_r4_after_870987_same_family_raw_null_generation_v7_route.py",
    "SCORE_ROWS=results/natural_evidence_v2/status/r4_after_870987_same_family_raw_null_v7_row_bank_plan_20260526/row_allocation_rows.jsonl",
    "ALLOCATION_ROWS=results/natural_evidence_v2/status/r4_after_870987_same_family_raw_null_v7_row_bank_plan_20260526/row_allocation_rows.jsonl",
    "TOKENIZER_REVIEW=results/natural_evidence_v2/status/r4_after_870987_same_family_raw_null_v7_tokenizer_preflight_877837_review/review_summary.json",
    "PUBLIC_RUN_SALT=r4_after_870987_same_family_raw_null_generation_v7",
    "SHARDS_PER_MODEL=15",
    "PLAN_ONLY=0",
    "VALIDATE_PLAN_ONLY=0",
    f"sbatch --array=0-44 {EXPECTED_WRAPPER}",
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate R4 same-family raw-null v7 generation route planning.")
...
def main() -> int:
    ...
    if cfg.get("schema_name") != "natural_evidence_v2_r4_after_870987_same_family_raw_null_generation_v7_route_v1":
        errors.append("schema_name mismatch")
    if cfg.get("route_id") != "r4_after_870987_same_family_raw_null_generation_v7":
    ...
    if not isinstance(source, Mapping):
        ...
        source = {}
    failed_summary = read_json(root_path(source.get("failed_generation_summary_877751", ""), errors, "877751 failed generation summary"))
    failed_review = read_json(root_path(source.get("failed_generation_review_877751", ""), errors, "877751 failure review"))
    row_manifest = read_json(root_path(source.get("v7_row_bank_rows", ""), errors, "v7 row bank rows").with_name("row_allocation_manifest.json"))
    row_validation = read_json(root_path(source.get("v7_row_bank_validation", ""), errors, "v7 row bank validation"))
    row_bank_rows = root_path(source.get("v7_row_bank_rows", ""), errors, "v7 row bank rows")
    tokenizer_review = read_json(root_path(source.get("same_family_v7_tokenizer_review", ""), errors, "same-family v7 tokenizer review"))

    if failed_summary.get("status") != "FAIL_R4_AFTER_870987_SAME_FAMILY_RAW_NULL_GENERATION_GATE":
        errors.append("877751 summary must remain failed")
    if failed_review.get("status") != "FAIL_R4_AFTER_870987_SAME_FAMILY_RAW_NULL_V6_877751_AMBIGUOUS_BUCKET_NO_ADOPT":
        errors.append("877751 failure review must remain no-adopt")
    if row_manifest.get("status") != "PASS_R4_AFTER_870987_SAME_FAMILY_RAW_NULL_V7_ROW_BANK_BUILT_ARTIFACT_ONLY_NO_SUBMIT":
        errors.append("v7 row bank manifest must pass")
    if row_validation.get("status") != "PASS_R4_AFTER_870987_SAME_FAMILY_RAW_NULL_V7_ROW_BANK_VALIDATION_NO_SUBMIT":
        errors.append("v7 row bank validation must pass")
    if tokenizer_review.get("status") != "PASS_R4_AFTER_870987_SAME_FAMILY_RAW_NULL_V7_TOKENIZER_PREFLIGHT_877837":
        errors.append("same-family v7 tokenizer review 877837 must pass")
    if int(tokenizer_review.get("checked_rows_total", -1)) != 46080:
        errors.append("same-family v7 tokenizer review must check 46080 rows")
    if int(tokenizer_review.get("failed_rows_total", -1)) != 0:
        errors.append("same-family v7 tokenizer review failed rows must be 0")
    if row_bank_rows.exists() and sum(1 for line in row_bank_rows.open("r", encoding="utf-8") if line.strip()) != 15360:
        errors.append("v7 row bank must contain 15360 rows")

    repair = cfg.get("repair_policy", {})
    if not isinstance(repair, Mapping):
        errors.append("repair_policy must be a mapping")
        repair = {}
    for key in ("rescue_877751", "relax_hard_forbidden_policy"):
    ...
    for item in models if isinstance(models, list) else []:
        if not isinstance(item, Mapping):
            ...
            continue
        if int(item.get("blocks", -1)) != 15:
            errors.append(f"{item.get('model_id')} blocks must be 15")
    ...
    if generator.exists():
        ...
        if EXPECTED_TOKENIZER_STATUS not in text:
            errors.append("generator missing v7 tokenizer review status in allowed-status list")
    ...
    status = (
        "PASS_R4_AFTER_870987_SAME_FAMILY_RAW_NULL_V7_GENERATION_ROUTE_PLAN_ONLY_NO_SUBMIT"
        if not errors
        else "FAIL_R4_AFTER_870987_SAME_FAMILY_RAW_NULL_V7_GENERATION_ROUTE_PLAN_ONLY_NO_SUBMIT"
    )
    summary = {
        "schema_name": "natural_evidence_v2_r4_after_870987_same_family_raw_null_v7_generation_route_validation_v1",
        "status": status,
        "errors": errors,
        "allowlist_entry": EXPECTED_ENTRY,
        "enabled_entries": enabled,
        "expected_models": expected_models,
        "expected_total_shards": 45,
        "expected_generated_rows": 46080,
        ...
        "tokenizer_review": source.get("same_family_v7_tokenizer_review", ""),
        "slurm_allowed": False,
        "generation_started": False,
        "paper_claim_allowed": False,
        "next_allowed_action": "Run local/remote hash preflight before exactly-one H200 same-family raw-null v7 generation submission.",
    }
