...
DEFAULT_CONFIG = ROOT / "configs/natural_evidence_v2/r4_after_870987_prefar_organic_null_generation_route.yaml"
ALLOWLIST = ROOT / "configs/natural_evidence_v2/run_allowlist.yaml"
EXPECTED_ENTRY = "v2_r4_after_870987_prefar_organic_null_generation_h200"
EXPECTED_WRAPPER = "scripts/natural_evidence_v2/slurm/r4_after_870987_prefar_organic_null_generation_h200.sbatch"
EXPECTED_COMMAND_PATTERN = f"PLAN_ONLY=0 VALIDATE_PLAN_ONLY=0 sbatch {EXPECTED_WRAPPER}"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate the R4 after-870987 pre-FAR organic-null raw-only generation route."
    )
...
def validate_route(
    ...
    *,
    ...
    errors: list[str] = []
    if config.get("schema_name") != "natural_evidence_v2_r4_after_870987_prefar_organic_null_generation_route_v1":
        errors.append("schema_name mismatch")
    if config.get("route_id") != "r4_after_870987_prefar_organic_null_generation_v1":
        errors.append("route_id mismatch")
    if (
        config.get("phase")
        != "V2_R4_AFTER_870987_PREFAR_ORGANIC_NULL_QWEN_TOKENIZER_PREFLIGHT_PASSED_GENERATION_ROUTE_PLANNING"
    ):
    ...
    standard_summary_path = root_path(
        source.get("standard_control_summary", ""), "source_artifacts.standard_control_summary", errors
    )
    tokenizer_review_path = root_path(
        source.get("tokenizer_preflight_review", ""), "source_artifacts.tokenizer_preflight_review", errors
    )
    prompt_validation_path = root_path(
        source.get("prompt_bank_validation", ""), "source_artifacts.prompt_bank_validation", errors
    )
    ...
    standard_summary = read_json(standard_summary_path) if standard_summary_path.exists() else {}
    ...
    if standard_summary.get("status") != "PASS_R4_AFTER_870987_PREFAR_STANDARD_CONTROL_GENERATION_GATE":
        errors.append("standard-control pre-FAR summary must pass before organic-null generation")
    combined = standard_summary.get("combined_standard_control_summary_by_arm", {})
    if not isinstance(combined, Mapping):
        errors.append("standard-control combined summary missing")
    else:
        for arm in ("raw", "task_only", "wrong_key", "wrong_payload"):
            item = combined.get(arm, {})
            if not isinstance(item, Mapping):
                errors.append(f"standard-control {arm} combined summary missing")
                continue
            if int_field(item, "total_blocks") != 256:
                errors.append(f"standard-control {arm} total_blocks must be 256")
            if int_field(item, "total_accepts") != 0 or int_field(item, "total_accepts_ignoring_quality") != 0:
                errors.append(f"standard-control {arm} must have zero accepts")

    if source.get("tokenizer_preflight_job_id") != "874307":
        errors.append("tokenizer_preflight_job_id must be 874307")
    if tokenizer_review.get("review_status") != "PASS_R4_AFTER_870987_PREFAR_ORGANIC_NULL_QWEN_TOKENIZER_PREFLIGHT_874307":
        errors.append("874307 tokenizer review must pass")
    for field, expected in (
        ...
        if int_field(tokenizer_review, field) != expected:
            errors.append(f"874307 tokenizer review {field} must be {expected}")
    for field in ("model_forward_pass_started", "generation_started", "training_started", "paper_claim_allowed"):
        if tokenizer_review.get(field) is not False:
            errors.append(f"874307 tokenizer review {field} must be false")

    if prompt_validation.get("status") != "PASS_R4_AFTER_870987_PREFAR_ORGANIC_NULL_PROMPT_BANK_VALIDATION_NO_SUBMIT":
        errors.append("organic-null prompt bank validation must pass")
    for field, expected in (
        ("locked_count", 16384),
        ("technical_hit_row_count", 0),
        ("structural_hit_row_count", 0),
        ("standard_control_prompt_overlap", 0),
        ("previous_locked_prompt_overlap", 0),
    ):
        if int_field(prompt_validation, field) != expected:
            errors.append(f"prompt validation {field} must be {expected}")
    ...
    if row_validation.get("status") != "PASS_R4_AFTER_870987_PREFAR_ORGANIC_NULL_ROW_BANK_VALIDATION_NO_SUBMIT":
        errors.append("organic-null row-bank validation must pass")
    for field, expected in (
        ("row_count", 262144),
        ("target_shards", 256),
        ("rows_per_shard", 1024),
        ("selected_prompt_count", 16384),
        ("selected_coordinate_count", 16),
        ("unique_content_prompt_prefix_pairs", 262144),
        ("unique_prompt_prefix_pairs", 262144),
    ...
    if manifest.get("organic_null") is not True or manifest.get("standard_control_null_expansion") is not False:
        errors.append("manifest must be organic-null, not standard-control")
    if manifest.get("source_standard_control_summary") != source.get("standard_control_summary"):
        errors.append("manifest source_standard_control_summary mismatch")
    if rows_path.exists() and sum(1 for line in rows_path.open("r", encoding="utf-8") if line.strip()) != 262144:
        errors.append("row bank rows must contain 262144 rows")
    ...
    for field, expected in (
        ...
        ("selected_coordinate_count", 16),
        ("expected_generated_rows", 262144),
        ("expected_attempt_rows_min", 262144),
    ):
        if int(scope.get(field, -1)) != expected:
            errors.append(f"generation_scope.{field} must be {expected}")
    if scope.get("conditions") != ["raw"] or scope.get("decode_conditions") != ["raw"]:
        errors.append("organic-null generation/decode conditions must be raw only")
    if scope.get("organic_null") is not True or scope.get("raw_only") is not True:
        errors.append("organic-null raw-only scope mismatch")
    ...
    for field, expected in (
        ("organic_null_blocks", 256),
        ("organic_null_accepts_max", 0),
        ("organic_null_accepts_ignoring_quality_max", 0),
    ):
        if int(targets.get(field, -1)) != expected:
            errors.append(f"null_package_targets.{field} must be {expected}")
    if targets.get("standard_control_summary_required_pass") is not True:
        errors.append("standard_control_summary_required_pass must be true")

    gates = mapping(config.get("quality_gates"), "quality_gates", errors)
    for field, expected in (
        ("raw_accepts_max", 0),
        ("raw_accepts_ignoring_quality_max", 0),
    ...
    if wrapper.exists():
        ...
        for fragment in (
            ...
            "r4_after_870987_prefar_organic_null_generation",
    ...
    status = (
        "PASS_R4_AFTER_870987_PREFAR_ORGANIC_NULL_GENERATION_ROUTE_PLAN_ONLY_NO_SUBMIT"
        if not errors
        else "FAIL_R4_AFTER_870987_PREFAR_ORGANIC_NULL_GENERATION_ROUTE_PLAN_ONLY_NO_SUBMIT"
    )
    return {
        "schema_name": "natural_evidence_v2_r4_after_870987_prefar_organic_null_generation_route_validation_v1",
        "status": status,
        "errors": errors,
        "tokenizer_preflight_job_id": "874307",
        ...
        "next_allowed_action": "Run local/remote hash preflight before exactly-one H200 pre-FAR organic-null generation submission.",
    }
