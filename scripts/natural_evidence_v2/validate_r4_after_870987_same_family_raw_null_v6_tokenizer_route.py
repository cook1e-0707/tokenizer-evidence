...
DEFAULT_CONFIG = ROOT / "configs/natural_evidence_v2/r4_after_870987_same_family_raw_null_v6_tokenizer_preflight_route.yaml"
...
EXPECTED_ROWS_PER_TOKENIZER = 16384


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate R4 same-family raw-null v6 tokenizer preflight route.")
...
def main() -> int:
    ...
    if cfg.get("schema_name") != "natural_evidence_v2_r4_after_870987_same_family_raw_null_v6_tokenizer_preflight_route_v1":
        errors.append("schema_name mismatch")
    if cfg.get("route_id") != "r4_after_870987_same_family_raw_null_v6_tokenizer_preflight_v1":
    ...
    row_manifest = read_json(root_path(source.get("v6_row_bank_manifest", ""), errors, "v6 row bank manifest"))
    ...
    if failed_review.get("status") != "FAIL_R4_AFTER_870987_SAME_FAMILY_RAW_NULL_V5_877142_FORBIDDEN_RESIDUAL_NO_ADOPT":
        errors.append("877142 review must remain no-adopt")
    ...
    if rows_path.exists() and sum(1 for line in rows_path.open("r", encoding="utf-8") if line.strip()) != EXPECTED_ROWS_PER_TOKENIZER:
        errors.append("v6 row bank must contain 65536 rows")
    ...
    for fragment in (
        "ROUTE_CONFIG=configs/natural_evidence_v2/r4_after_870987_same_family_raw_null_v6_tokenizer_preflight_route.yaml",
        ...
        "MAX_ROWS=16384",
        "ROUTE_VALIDATOR=scripts/natural_evidence_v2/validate_r4_after_870987_same_family_raw_null_v6_tokenizer_route.py",
    ...
    if entry is None:
        ...
        for fragment in (
            "r4_after_870987_same_family_raw_null_v6_tokenizer_preflight_route.yaml",
            "r4_after_870987_same_family_raw_null_v6_row_bank_plan_20260525/row_allocation_rows.jsonl",
            "validate_r4_after_870987_same_family_raw_null_v6_tokenizer_route.py",
        ):
            if fragment not in entry_command:
                errors.append(f"allowlist command_pattern missing fragment: {fragment}")

    status = (
        "PASS_R4_AFTER_870987_SAME_FAMILY_RAW_NULL_V6_TOKENIZER_ROUTE_PLAN_ONLY_NO_SUBMIT"
        if not errors
        else "FAIL_R4_AFTER_870987_SAME_FAMILY_RAW_NULL_V6_TOKENIZER_ROUTE_PLAN_ONLY_NO_SUBMIT"
    )
    ...
    summary = {
        "schema_name": "natural_evidence_v2_r4_after_870987_same_family_raw_null_v6_tokenizer_route_validation_v1",
        "status": status,
        "errors": errors,
    ...
    write_text_new(
        ...
        "# R4 Same-Family Raw-Null V6 Tokenizer Route Validation\n\n"
        f"Status: `{status}`\n\n"
        f"Errors: {len(errors)}\n\n"
