...
DEFAULT_CONFIG = ROOT / "configs/natural_evidence_v2/r4_after_868016_reliability_coordinate_pivot_tokenizer_route.yaml"
ALLOWLIST = ROOT / "configs/natural_evidence_v2/run_allowlist.yaml"
EXPECTED_ENTRY = "v2_r4_after_868016_reliability_coordinate_pivot_qwen_tokenizer_boundary_preflight_h200"
EXPECTED_WRAPPER = (
    "scripts/natural_evidence_v2/slurm/"
    "r4_after_868016_reliability_coordinate_pivot_qwen_tokenizer_boundary_preflight_h200.sbatch"
)
EXPECTED_ROWS = 3072
EXPECTED_EXCLUDED = [3, 10, 20, 24]
...
def validate_route(config: Mapping[str, Any]) -> dict[str, Any]:
    errors: list[str] = []
    if config.get("schema_name") != "natural_evidence_v2_r4_after_868016_reliability_coordinate_pivot_tokenizer_route_v1":
        errors.append("schema_name mismatch")
    if config.get("package_id") != "r4_after_868016_reliability_coordinate_pivot_tokenizer_route_v1":
    ...
    if source_failure.get("job_id") != "868016":
        errors.append("source_failure.job_id must be 868016")
    ...
    if failure.get("posthoc_exclusion_is_diagnostic_only") is not True:
        errors.append("source_failure posthoc exclusion must be diagnostic-only")
    ...
    if int(rows_summary.get("row_count", 0)) != EXPECTED_ROWS:
        errors.append("source_rows row_count must be 3072")
    if int(source_rows.get("expected_rows", 0)) != EXPECTED_ROWS:
        errors.append("source_rows.expected_rows must be 3072")
    ...
    if int(rows_summary.get("selected_coordinate_count", 0)) != 12:
        errors.append("source_rows selected_coordinate_count must be 12")
    if int(rows_summary.get("prompt_offset", -1)) != int(source_rows.get("expected_prompt_offset", -2)):
        errors.append("source_rows prompt_offset mismatch")
    if list(rows_summary.get("excluded_coordinates", [])) != list(source_rows.get("expected_excluded_coordinates", [])):
        errors.append("source_rows excluded_coordinates mismatch")
    if list(source_rows.get("expected_excluded_coordinates", [])) != EXPECTED_EXCLUDED:
        errors.append("source_rows expected excluded coordinates mismatch")
    ...
    if rows_path.exists():
        ...
        if row_count != EXPECTED_ROWS:
            errors.append("source_rows JSONL line count must be 3072")
    ...
    if int(static_summary.get("checked_row_count", 0)) != EXPECTED_ROWS:
        errors.append("source_static_preflight checked_row_count must be 3072")
    ...
    if int(route.get("max_rows", 0)) != EXPECTED_ROWS:
        errors.append("route.max_rows must be 3072")
    ...
    required_wrapper_fragments = (
        ...
        "r4_after_868016_reliability_coordinate_pivot_rows_20260516/reliability_surface_mass_rows.jsonl",
    ...
    if int(gate.get("checked_rows", 0)) != EXPECTED_ROWS:
        errors.append("future_tokenizer_gate.checked_rows must be 3072")
    ...
    if future.get("next_route") != "reviewed_h200_teacher_forced_controller_scoring_only":
    ...
    status = (
        "PASS_R4_AFTER_868016_RELIABILITY_COORDINATE_PIVOT_TOKENIZER_ROUTE_VALIDATION_NO_SUBMIT"
        if not errors
        else "FAIL_R4_AFTER_868016_RELIABILITY_COORDINATE_PIVOT_TOKENIZER_ROUTE_VALIDATION_NO_SUBMIT"
    )
    return {
        "status": status,
        "errors": errors,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate after-868016 coordinate-pivot tokenizer preflight route.")
...
def main() -> int:
    ...
    if args.output_dir is not None:
        write_json_new(args.output_dir / "coordinate_pivot_tokenizer_route_validation_summary.json", summary)
