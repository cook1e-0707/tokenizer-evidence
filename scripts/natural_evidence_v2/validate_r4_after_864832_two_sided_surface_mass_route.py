...
DEFAULT_CONFIG = ROOT / "configs/natural_evidence_v2/r4_after_864832_two_sided_surface_mass_score_route.yaml"
ALLOWLIST = ROOT / "configs/natural_evidence_v2/run_allowlist.yaml"
EXPECTED_ENTRY = "v2_r4_after_864832_two_sided_surface_mass_score_h200"
EXPECTED_WRAPPER = "scripts/natural_evidence_v2/slurm/r4_after_864832_two_sided_surface_mass_score_h200.sbatch"
...
def validate_route(config: Mapping[str, Any]) -> dict[str, Any]:
    errors: list[str] = []
    if config.get("schema_name") != "natural_evidence_v2_r4_after_864832_two_sided_surface_mass_score_route_v1":
        errors.append("schema_name mismatch")

    tok = _mapping(config.get("source_tokenizer_preflight"), "source_tokenizer_preflight", errors)
    tok_summary_path = _path(tok.get("review_summary", ""), "source_tokenizer_preflight.review_summary", errors)
    tok_summary = _read_json(tok_summary_path, "source_tokenizer_preflight.review_summary", errors) if tok_summary_path.exists() else {}
    ...
    if int(rows_summary.get("row_count", -1)) != int(rows.get("expected_rows", -2)):
        errors.append("source rows summary count mismatch")
    if rows_path.exists() and sum(1 for _ in rows_path.open("r", encoding="utf-8")) != int(rows.get("expected_rows", -1)):
    ...
    status = (
        ...
        if not errors
        else "FAIL_R4_AFTER_864832_TWO_SIDED_SURFACE_MASS_ROUTE_VALIDATION_NO_SUBMIT"
    )
    return {
        "status": status,
        "errors": errors,
        ...
        "expected_rows": rows.get("expected_rows"),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate R4 after-864832 two-sided surface-mass scoring route.")
...
def main() -> int:
    ...
    if args.output_dir is not None:
        write_json_new(args.output_dir / "two_sided_surface_mass_route_validation_summary.json", summary)
