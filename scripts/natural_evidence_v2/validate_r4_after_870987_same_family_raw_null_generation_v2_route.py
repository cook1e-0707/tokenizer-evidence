...
DEFAULT_CONFIG = ROOT / "configs/natural_evidence_v2/r4_after_870987_same_family_raw_null_generation_v2_route.yaml"
...
EXPECTED_COMMAND_FRAGMENTS = (
    "ROUTE_CONFIG=configs/natural_evidence_v2/r4_after_870987_same_family_raw_null_generation_v2_route.yaml",
    "ROUTE_VALIDATOR=scripts/natural_evidence_v2/validate_r4_after_870987_same_family_raw_null_generation_v2_route.py",
    "SCORE_ROWS=results/natural_evidence_v2/status/r4_after_870987_same_family_raw_null_v2_row_bank_plan_20260523/row_allocation_rows.jsonl",
    "ALLOCATION_ROWS=results/natural_evidence_v2/status/r4_after_870987_same_family_raw_null_v2_row_bank_plan_20260523/row_allocation_rows.jsonl",
    "TOKENIZER_REVIEW=results/natural_evidence_v2/status/r4_after_870987_same_family_raw_null_v2_tokenizer_preflight_875427_review/review_summary.json",
    "PUBLIC_RUN_SALT=r4_after_870987_same_family_raw_null_generation_v2",
    "PLAN_ONLY=0",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate R4 same-family raw-null v2 generation route planning.")
...
def main() -> int:
    ...
    if cfg.get("schema_name") != "natural_evidence_v2_r4_after_870987_same_family_raw_null_generation_v2_route_v1":
        errors.append("schema_name mismatch")
    if cfg.get("route_id") != "r4_after_870987_same_family_raw_null_generation_v2":
    ...
    if not isinstance(source, Mapping):
        ...
        source = {}
    failed_summary = read_json(root_path(source.get("failed_generation_summary", ""), errors, "failed generation summary"))
    failed_review = read_json(root_path(source.get("failed_generation_review", ""), errors, "failed generation review"))
    row_manifest = read_json(root_path(source.get("v2_row_bank_rows", ""), errors, "v2 row bank rows").with_name("row_allocation_manifest.json"))
    row_validation = read_json(root_path(source.get("v2_row_bank_validation", ""), errors, "v2 row bank validation"))
    row_bank_rows = root_path(source.get("v2_row_bank_rows", ""), errors, "v2 row bank rows")
    tokenizer_review = read_json(root_path(source.get("same_family_v2_tokenizer_review", ""), errors, "same-family v2 tokenizer review"))

    if failed_summary.get("status") != "FAIL_R4_AFTER_870987_SAME_FAMILY_RAW_NULL_GENERATION_GATE":
        errors.append("875168 failed generation summary must remain failed")
    if failed_review.get("status") != "FAIL_R4_AFTER_870987_SAME_FAMILY_RAW_NULL_GENERATION_GATE_NO_ADOPT":
        errors.append("875168 failure review must remain no-adopt")
    if row_manifest.get("status") != "PASS_R4_AFTER_870987_SAME_FAMILY_RAW_NULL_V2_ROW_BANK_BUILT_ARTIFACT_ONLY_NO_SUBMIT":
        errors.append("v2 row bank manifest must pass")
    if row_validation.get("status") != "PASS_R4_AFTER_870987_SAME_FAMILY_RAW_NULL_V2_ROW_BANK_VALIDATION_NO_SUBMIT":
        errors.append("v2 row bank validation must pass")
    if tokenizer_review.get("review_status") != "PASS_R4_AFTER_870987_SAME_FAMILY_RAW_NULL_V2_TOKENIZER_PREFLIGHT_875427":
        errors.append("same-family v2 tokenizer review 875427 must pass")
    if int(tokenizer_review.get("checked_rows_total", -1)) != 196608:
        errors.append("same-family v2 tokenizer review must check 196608 rows")
    if row_bank_rows.exists() and sum(1 for line in row_bank_rows.open("r", encoding="utf-8") if line.strip()) != 65536:
        errors.append("v2 row bank must contain 65536 rows")

    repair = cfg.get("repair_policy", {})
    if not isinstance(repair, Mapping):
        errors.append("repair_policy must be a mapping")
        repair = {}
    for key in ("rescue_875168", "relax_hard_forbidden_policy"):
        if repair.get(key) is not False:
            errors.append(f"repair_policy.{key} must be false")
    for key in ("exclude_875168_collision_prompt_ids", "exclude_collision_prompt_domains", "static_lexical_preflight_required"):
    ...
    status = (
        ...
        if not errors
        else "FAIL_R4_AFTER_870987_SAME_FAMILY_RAW_NULL_V2_GENERATION_ROUTE_PLAN_ONLY_NO_SUBMIT"
    )
    summary = {
        "schema_name": "natural_evidence_v2_r4_after_870987_same_family_raw_null_v2_generation_route_validation_v1",
        "status": status,
        "errors": errors,
        "allowlist_entry": EXPECTED_ENTRY,
        ...
        "tokenizer_review": source.get("same_family_v2_tokenizer_review", ""),
        "slurm_allowed": False,
        "generation_started": False,
        "paper_claim_allowed": False,
        "next_allowed_action": "Run local/remote hash preflight before exactly-one H200 same-family raw-null v2 generation submission.",
    }
