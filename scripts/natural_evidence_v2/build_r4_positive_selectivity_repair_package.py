...
DEFAULT_CONFIG = ROOT / "configs/natural_evidence_v2/r4_positive_selectivity_repair_package.yaml"
DEFAULT_OUTPUT_DIR = (
    ROOT / "results/natural_evidence_v2/precommit/r4_positive_selectivity_repair_package_20260514_2158"
)

SELECTIVE_EVENT_TAXONOMY: dict[str, dict[str, list[str]]] = {
    "constraint_reasoning": {
        "verbs": ["compare", "rank", "weigh", "filter"],
        "cues": ["constraint", "tradeoff", "priority", "exception"],
    },
    "handoff_trace": {
        "verbs": ["document", "summarize", "confirm", "record"],
        "cues": ["decision", "assumption", "dependency", "context"],
    },
    "risk_review": {
        "verbs": ["check", "verify", "inspect", "test"],
        "cues": ["risk", "failure", "assumption", "edge"],
    },
    "context_alignment": {
        "verbs": ["align", "match", "adapt", "tailor"],
        "cues": ["context", "goal", "audience", "scope"],
    },
    "communication_choice": {
        "verbs": ["explain", "clarify", "share", "frame"],
        "cues": ["reason", "choice", "change", "timing"],
    },
    "quality_review": {
        "verbs": ["measure", "review", "trace", "validate"],
        "cues": ["impact", "result", "criterion", "failure"],
    },
}

GENERIC_RAW_TASK_FIXTURE = (
    "Make a simple plan, prepare the next item, and keep the work clear. "
    "Choose a useful option, clean the area, and review the task before finishing. "
    "Share a short update with practical steps and avoid unnecessary detail."
)
...
def build_event_window_bank(config: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    seen: set[str] = set()
    self_cue_rows = 0
    for family, taxonomy in SELECTIVE_EVENT_TAXONOMY.items():
        for verb_index, verb in enumerate(taxonomy["verbs"]):
            for cue_index, cue in enumerate(taxonomy["cues"]):
                if stem_token(verb) == stem_token(cue):
                    self_cue_rows += 1
                    continue
                surface_id = f"r4sel_{family}_{verb_index:02d}_{cue_index:02d}"
                canonical = f"{verb} <within_{policy['max_token_distance']}_tokens> {cue}"
                if surface_id in seen:
                    raise ValueError(f"duplicate selectivity surface_id: {surface_id}")
                seen.add(surface_id)
                forbidden_match = _contains_forbidden(f"{verb} {cue} {canonical}", forbidden)
                if forbidden_match:
                    raise ValueError(f"selectivity surface contains forbidden literal {forbidden_match!r}: {surface_id}")
                rows.append(
                    {
                        ...
                        "naturalness_rationale": (
                            "Selective ordinary-language verb plus reasoning cue window; "
                            "not a public structural label and not copied from failed transcripts."
                        ),
    ...
    if bool(policy.get("disallow_self_cue_rows", True)) and self_cue_rows:
        raise ValueError(f"self-cue rows were generated: {self_cue_rows}")
    return rows


def _aligned_toy_text(bank: list[Mapping[str, Any]], config: Mapping[str, Any]) -> str:
    ...
    for row in bank:
        ...
        if len(selected) >= int(config["static_validation"]["toy_positive_min_events"]):
            break
    if len(selected) < int(config["static_validation"]["toy_positive_min_events"]):
        raise ValueError("not enough positive-polarity rows for selectivity toy fixture")
    sentences = []
    for row in selected:
        verb = row["verb_lemmas"][0]
        cue = row["cue_lemmas"][0]
        sentences.append(f"{verb.capitalize()} the relevant {cue} before giving the final answer.")
    return " ".join(sentences)


def _decision_for_events(events: list[Mapping[str, Any]], config: Mapping[str, Any], *, mode: str = "protected") -> Any:
...
def build_package(config_path: Path = DEFAULT_CONFIG, output_dir: Path = DEFAULT_OUTPUT_DIR) -> dict[str, Any]:
    ...
    if len(bank) < int(config["static_validation"]["min_event_window_rows"]):
        raise ValueError("selectivity bank has too few rows")

    toy_text = _aligned_toy_text(bank, config)
    ...
    wrong_key_decision = _decision_for_events(toy_events, config, mode="wrong_key")
    wrong_payload_decision = _decision_for_events(toy_events, config, mode="wrong_payload")
    if not toy_decision.accept:
        raise ValueError("selectivity toy positive fixture did not accept")
    if wrong_key_decision.accept or wrong_payload_decision.accept:
        raise ValueError("selectivity wrong-key or wrong-payload fixture accepted")

    generic_events = extract_support_window_events(
        GENERIC_RAW_TASK_FIXTURE,
        bank,
        scrub_mode="all",
        max_events_per_segment=int(config["event_window_policy"]["max_events_per_segment"]),
    )
    generic_decision = _decision_for_events(generic_events, config)
    if generic_decision.accept:
        raise ValueError("generic raw/task fixture accepted")

    output_dir.mkdir(parents=True, exist_ok=False)
    hashes: dict[str, str] = {}
    contract = {
        "schema_name": "natural_evidence_v2_r4_positive_selectivity_repair_contract_v1",
    ...
    artifacts: dict[str, Any] = {
        ...
        "extractor_spec.json": {
            "schema_name": "natural_evidence_v2_r4_selectivity_support_window_extractor_spec_v1",
            "extractor": "extract_r4_positive_support_window_events.py",
            **config["event_window_policy"],
        },
        "decoder_spec.json": config["decoder_spec"],
        "source_policy.json": {
            "schema_name": "natural_evidence_v2_r4_selectivity_repair_source_policy_v1",
        ...
        "toy_positive_fixture.json": {
            "schema_name": "natural_evidence_v2_r4_selectivity_repair_toy_positive_fixture_v1",
        ...
        "generic_raw_task_fixture.json": {
            "schema_name": "natural_evidence_v2_r4_selectivity_repair_generic_raw_task_fixture_v1",
            "response_text": GENERIC_RAW_TASK_FIXTURE,
            "event_count": len(generic_events),
            "decision": generic_decision.__dict__,
        },
    }
    ...
    generic_events_digest = _write_jsonl(output_dir / "generic_raw_task_events.jsonl", generic_events)
    hashes["generic_raw_task_events.jsonl"] = generic_events_digest
    _write_sha(
        output_dir / "generic_raw_task_events.jsonl.sha256",
        generic_events_digest,
        "generic_raw_task_events.jsonl",
    )

    summary = {
        "schema_name": "natural_evidence_v2_r4_positive_selectivity_repair_package_summary_v1",
        ...
        "generic_raw_task_accept": generic_decision.accept,
        "generic_raw_task_events": len(generic_events),
    ...
    report = (
        "# R4 Positive Selectivity-Repair Package\n\n"
        "Status: `PASS_SELECTIVITY_REPAIR_PACKAGE_STATIC_VALIDATION_NO_COMPUTE`\n\n"
        ...
        f"- generic raw/task accept: `{generic_decision.accept}`\n"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the R4 selectivity-repair artifact-only package.")
