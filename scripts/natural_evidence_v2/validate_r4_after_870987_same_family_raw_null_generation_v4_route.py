...
DEFAULT_CONFIG = ROOT / "configs/natural_evidence_v2/r4_after_870987_same_family_raw_null_generation_v4_route.yaml"
...
EXPECTED_COMMAND_FRAGMENTS = (
    "ROUTE_CONFIG=configs/natural_evidence_v2/r4_after_870987_same_family_raw_null_generation_v4_route.yaml",
    "ROUTE_VALIDATOR=scripts/natural_evidence_v2/validate_r4_after_870987_same_family_raw_null_generation_v4_route.py",
    "SCORE_ROWS=results/natural_evidence_v2/status/r4_after_870987_same_family_raw_null_v4_row_bank_plan_20260524/row_allocation_rows.jsonl",
    "ALLOCATION_ROWS=results/natural_evidence_v2/status/r4_after_870987_same_family_raw_null_v4_row_bank_plan_20260524/row_allocation_rows.jsonl",
    "TOKENIZER_REVIEW=results/natural_evidence_v2/status/r4_after_870987_same_family_raw_null_v4_tokenizer_preflight_876849_review/review_summary.json",
    "PUBLIC_RUN_SALT=r4_after_870987_same_family_raw_null_generation_v4",
    "PLAN_ONLY=0",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate R4 same-family raw-null v4 generation route planning.")
...
def main() -> int:
    ...
    if cfg.get("schema_name") != "natural_evidence_v2_r4_after_870987_same_family_raw_null_generation_v4_route_v1":
        errors.append("schema_name mismatch")
    if cfg.get("route_id") != "r4_after_870987_same_family_raw_null_generation_v4":
    ...
    if not isinstance(source, Mapping):
        ...
        source = {}
    failed_summary = read_json(root_path(source.get("failed_generation_summary_875777", ""), errors, "875777 failed generation summary"))
    failed_review = read_json(root_path(source.get("failed_generation_review_875777", ""), errors, "875777 failure review"))
    row_manifest = read_json(root_path(source.get("v4_row_bank_rows", ""), errors, "v4 row bank rows").with_name("row_allocation_manifest.json"))
    row_validation = read_json(root_path(source.get("v4_row_bank_validation", ""), errors, "v4 row bank validation"))
    row_bank_rows = root_path(source.get("v4_row_bank_rows", ""), errors, "v4 row bank rows")
    tokenizer_review = read_json(root_path(source.get("same_family_v4_tokenizer_review", ""), errors, "same-family v4 tokenizer review"))

    if failed_summary.get("status") != "FAIL_R4_AFTER_870987_SAME_FAMILY_RAW_NULL_GENERATION_GATE":
        errors.append("875777 summary must remain failed")
    if failed_review.get("status") != "FAIL_R4_AFTER_870987_SAME_FAMILY_RAW_NULL_V3_875777_FORBIDDEN_RESIDUAL_NO_ADOPT":
        errors.append("875777 failure review must remain no-adopt")
    if row_manifest.get("status") != "PASS_R4_AFTER_870987_SAME_FAMILY_RAW_NULL_V4_ROW_BANK_BUILT_ARTIFACT_ONLY_NO_SUBMIT":
        errors.append("v4 row bank manifest must pass")
    if row_validation.get("status") != "PASS_R4_AFTER_870987_SAME_FAMILY_RAW_NULL_V4_ROW_BANK_VALIDATION_NO_SUBMIT":
        errors.append("v4 row bank validation must pass")
    if tokenizer_review.get("status") != "PASS_R4_AFTER_870987_SAME_FAMILY_RAW_NULL_V4_TOKENIZER_PREFLIGHT_876849":
        errors.append("same-family v4 tokenizer review 876849 must pass")
    if int(tokenizer_review.get("checked_rows_total", -1)) != 196608:
        errors.append("same-family v4 tokenizer review must check 196608 rows")
    if int(tokenizer_review.get("failed_rows_total", -1)) != 0:
        errors.append("same-family v4 tokenizer review failed rows must be 0")
    if row_bank_rows.exists() and sum(1 for line in row_bank_rows.open("r", encoding="utf-8") if line.strip()) != 65536:
        errors.append("v4 row bank must contain 65536 rows")

    repair = cfg.get("repair_policy", {})
    if not isinstance(repair, Mapping):
        errors.append("repair_policy must be a mapping")
        repair = {}
    for key in ("rescue_875777", "relax_hard_forbidden_policy"):
    ...
    status = (
        ...
        if not errors
        else "FAIL_R4_AFTER_870987_SAME_FAMILY_RAW_NULL_V4_GENERATION_ROUTE_PLAN_ONLY_NO_SUBMIT"
    )
    summary = {
        "schema_name": "natural_evidence_v2_r4_after_870987_same_family_raw_null_v4_generation_route_validation_v1",
        "status": status,
        "errors": errors,
        "allowlist_entry": EXPECTED_ENTRY,
        ...
        "tokenizer_review": source.get("same_family_v4_tokenizer_review", ""),
        "slurm_allowed": False,
        "generation_started": False,
        "paper_claim_allowed": False,
        "next_allowed_action": "Run local/remote hash preflight before exactly-one H200 same-family raw-null v4 generation submission.",
    }
