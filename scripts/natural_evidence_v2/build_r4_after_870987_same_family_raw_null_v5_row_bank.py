...
DEFAULT_OUTPUT_DIR = (
    ROOT
    / "results/natural_evidence_v2/status/"
    / "r4_after_870987_same_family_raw_null_v5_row_bank_plan_20260525"
)
DEFAULT_VALIDATION_DIR = (
    ROOT
    / "results/natural_evidence_v2/status/"
    / "r4_after_870987_same_family_raw_null_v5_row_bank_validation_20260525"
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build and validate artifact-only R4 same-family raw-null v5 row bank "
            "after 876852 residual forbidden failure."
        )
    )
...
def patch_rows_to_v5(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    patched: list[dict[str, Any]] = []
    for row in rows:
        new_row = dict(row)
        row_key = str(new_row.get("row_key", ""))
        for old in ("sfrawv3|", "sfrawv4|"):
            if row_key.startswith(old):
                row_key = row_key.replace(old, "sfrawv5|", 1)
                break
        new_row.update(
            {
                "schema_name": "natural_evidence_v2_r4_after_870987_same_family_raw_null_v5_row_bank_row_v1",
                "artifact_role": "r4_after_870987_same_family_raw_null_v5_row_bank_not_tokenized_not_scored",
                "same_family_raw_null_v3": False,
                "same_family_raw_null_v4": False,
                "same_family_raw_null_v5": True,
                "source_failure_job_ids": ["875168", "875471", "875777", "876852"],
                "allocation_policy": "same_family_raw_null_v5_residual_forbidden_literal_repair",
                "row_key": row_key,
            }
        )
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    text = f"""# R4 Same-Family Raw-Null V5 Row Bank Plan

Status: `{summary['status']}`

This artifact-only package repairs the residual forbidden-surface failure from
`876852`. It does not reinterpret `876852`, relax the hard forbidden policy,
...
def main() -> int:
    ...
    selected_rows = patch_rows_to_v5(
    ...
    validation["schema_name"] = "natural_evidence_v2_r4_after_870987_same_family_raw_null_v5_row_bank_validation_v1"
    validation["status"] = validation["status"].replace("_V3_", "_V5_")
    validation["next_allowed_action"] = (
        "If reviewed, prepare actual Qwen tokenizer preflight route for same-family raw-null v5; "
        "no generation submission yet."
    )

    manifest = {
        "schema_name": "natural_evidence_v2_r4_after_870987_same_family_raw_null_v5_row_bank_manifest_v1",
        "status": "PASS_R4_AFTER_870987_SAME_FAMILY_RAW_NULL_V5_ROW_BANK_BUILT_ARTIFACT_ONLY_NO_SUBMIT",
        ...
        "next_allowed_action": "review this v5 row bank and validation; then prepare tokenizer preflight route if accepted",
    }
    ...
    write_text_new(
        ...
        "# R4 Same-Family Raw-Null V5 Row Bank Validation\n\n"
