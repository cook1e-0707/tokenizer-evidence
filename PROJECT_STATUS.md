...
- Active execution scope:
  - manuscript-facing consolidation, C1 reporting, and baseline/calibration protocol preparation only
...
2. Move next to `B0`: freeze matched-budget benchmark and calibration protocols before any baseline execution.
...
4. Keep `B1`/`B2` matched-budget baselines blocked until `B0` is frozen; keep `R1` second-family replication blocked until the baseline protocol is stable.
...
- Next target: freeze `B0` matched-budget benchmark and calibration protocols before any baseline execution.
