...
| B1 | Active fingerprint baseline comparison | pending package artifacts exist: fixed representative, uniform bucket, and English-random active fingerprint rows are staged under frozen B0; calibration train/eval manifests exist for fixed representative and uniform bucket only; no standing baseline result yet | real calibration scores, frozen thresholds, `foundation_null`/`organic_prompt_null` materialization or scoped blocker decision, and non-placeholder active-fingerprint integration | `configs/reporting/matched_budget_baselines_v1.yaml`; `manifests/matched_budget_baselines/*.json`; `results/tables/matched_budget_baselines.csv`; `results/tables/baseline_calibration_cases.csv`; `results/processed/paper_stats/baseline_summary.json`; `results/processed/paper_stats/baseline_calibration_summary.json` | Local prepare, Chimera later | Final execution blocked until calibration thresholds are frozen and placeholder rows are resolved or explicitly unavailable | P2 |
| B2 | Provenance / watermark control comparison | pending package artifacts exist for KGW as a task-mismatched provenance control; no standing control result yet | real control implementation or explicit unavailable status under frozen B0 | `configs/reporting/matched_budget_baselines_v1.yaml`; `manifests/matched_budget_baselines/*.json`; `results/tables/matched_budget_baselines.csv`; `results/processed/paper_stats/baseline_summary.json` | Local prepare, Chimera later | Must remain labeled task-mismatched control; final execution blocked until calibration thresholds are frozen | P2 |
...
6. `B1` + `B2`: minimal baseline comparison package is prepared as pending artifacts; run calibration manifests first, then run final only after real calibration scores freeze thresholds and placeholder external baselines are integrated or explicitly left unavailable.
