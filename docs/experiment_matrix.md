...
| G1 | Main-path scale package: payload × seed coverage | publication-scale Qwen7B clean package is now standing on `U00..U15 @ seeds 17,23,29`; final aggregate is `48/48` included, `0` pending, `0` excluded | none for the current Qwen7B payload x seed package beyond freeze-and-cite manuscript alignment | `configs/experiment/scale/exp_train__qwen2_5_7b__g1_payload_seed_scale_v1.yaml`; `configs/experiment/scale/exp_eval__qwen2_5_7b__g1_payload_seed_scale_v1.yaml`; `results/tables/g1_payload_seed_scale.csv`; `results/tables/g1_payload_seed_scale.tex`; `results/processed/paper_stats/g1_summary.json`; `results/processed/paper_stats/g1_run_inclusion_list.json` | Local review only | Freeze current package; any follow-on scale work should start at `G2` without changing the G1 codebook or model family | P0 |
| G2 | Prompt-family scale package | currently only one standing prompt contract is paper-facing | controlled prompt-family robustness package (same semantics, different surface forms) | `configs/experiment/scale/exp_qwen7b_prompt_family_*`; `results/tables/g2_prompt_family.csv`; `results/processed/paper_stats/g2_summary.json` | Local prepare, Chimera run | Keep same codebook and model; prompt changes must preserve semantics | P1 |
| G3 | Codebook / slot / block scale package | current standing path uses one main compiled codebook and block_count up to 2 | controlled scaling over bucket cardinality and block count to answer “toy codebook” criticism | `configs/experiment/scale/exp_qwen7b_codebook_scale_*`; `results/tables/g3_codebook_scale.csv`; `results/processed/paper_stats/g3_summary.json` | Local prepare, Chimera run | Must remain on Qwen7B; do not open new attack families yet | P1 |
...
5. `G2` -> `G3` -> `G4`: run the remaining publication-scale packages on Qwen7B while keeping `G1` frozen.
