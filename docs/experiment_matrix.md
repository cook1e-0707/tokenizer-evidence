...
| T2 | Theorem 2 / objective comparison package (`Section 5`) | prep configs exist and a direct-submit Chimera package can now be opened on the same compiled path | actual Chimera evidence for `bucket_mass` vs `fixed_representative` vs `uniform_bucket` | `configs/experiment/prep/exp_train__qwen2_5_7b__t2_*.yaml`; `configs/experiment/prep/exp_eval__qwen2_5_7b__t2_*.yaml`; `docs/t2_chimera_package.md`; `results/processed/paper_stats/theorem_package_dry_runs.json` | Local prepare complete, Chimera next | Same codebook; same model; no baseline opening; no full Chimera sweep yet | P1 |
