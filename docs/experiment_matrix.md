...
| G4 | Training-signal scale package | package prepared locally with fixed Qwen7B, B2, codebook, prompt family, G3a-v3 hp04 objective, and `S16/S32/S64/S128 x U00/U03/U12/U15 x seeds 17,23,29` dry-run manifests | Chimera execution for `48 train + 48 eval`; then rebuild `g4_train_scale` artifacts from scratch outputs | `configs/experiment/scale/g4/exp_train__qwen2_5_7b__g4_train_signal_scale_v1.yaml`; `configs/experiment/scale/g4/exp_eval__qwen2_5_7b__g4_train_signal_scale_v1.yaml`; `configs/reporting/g4_train_signal_scale_v1.yaml`; `manifests/g4_qwen7b_train_signal_scale/train_manifest.json`; `manifests/g4_qwen7b_train_signal_scale/eval_manifest.json`; `results/tables/g4_train_scale.csv`; `results/tables/g4_train_scale.tex`; `results/processed/paper_stats/g4_summary.json`; `results/processed/paper_stats/g4_run_inclusion_list.json` | Local prepare complete, Chimera run next | Only effective training-signal sample count varies; `S128` is explicitly `64` unique compiled samples repeated twice under `compiled_sample_repeats=2` | P1 |
| C1 | Statistical reporting / compute accounting / reproducibility | G3a-v3 standing compute/stat rows are now represented; G4 pending package emits its own compute estimate and run inclusion schema | after G4 Chimera completion, regenerate `g4_summary`, then merge G4 rows into canonical C1 | `results/processed/paper_stats/compute_accounting.json`; `results/processed/paper_stats/run_inclusion_lists.json`; `results/tables/stat_summary.csv`; `results/tables/compute_accounting.csv`; package-local `*_compute_accounting.json` | Local | Must separate validation/preliminary compute from final reported package compute and keep valid method failures in denominators | P1 |
...
5. `G4`: run the training-signal scale package on Qwen7B while keeping `G1`, `G2`, and `G3a-v3` frozen.
