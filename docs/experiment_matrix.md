...
| T1 | Theorem 1 / tokenizer-contextual alignment package (`Section 4`) | `theorem1_qwen7b/contextual_exact` passed on Chimera; `sequence_proxy` train completed but eval is blocked by `compiled_gate` expecting `compiled_slot_values` while the control arm emits `scaffolded_slot_values` | repaired control-arm eval package so the contextual and proxy arms are directly comparable under the same payload/codebook | `configs/experiment/prep/exp_train__qwen2_5_7b__t1_*.yaml`; `configs/experiment/prep/exp_eval__qwen2_5_7b__t1_*.yaml`; `docs/t1_chimera_package.md`; `results/processed/paper_stats/theorem_package_dry_runs.json` | Local prepare, Chimera after repair | Same `Qwen/Qwen2.5-7B-Instruct`; same compiled codebook; no sweep yet | P1 |
| T2 | Theorem 2 / objective comparison package (`Section 5`) | initial `T2` Chimera package executed, but all three objectives collapsed to the same effective supervision problem under the single-token-per-bucket compiled catalog | actual discriminative Chimera evidence for `bucket_mass` vs `fixed_representative` vs `uniform_bucket` under a controlled multi-member-bucket Qwen catalog | `configs/experiment/prep/exp_train__qwen2_5_7b__t2r1_*.yaml`; `configs/experiment/prep/exp_eval__qwen2_5_7b__t2r1_*.yaml`; `docs/t2_chimera_package.md`; `results/processed/paper_stats/theorem_package_dry_runs.json` | Local prepare complete, Chimera next | Same model family; controlled theorem contrast allowed; no baseline opening; no full Chimera sweep yet | P1 |
