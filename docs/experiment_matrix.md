...
| B0 | Benchmark protocol / matched-budget calibration | not yet built | benchmark protocol, calibration plan, null FAR protocol, utility protocol | `docs/baseline_protocol.md`; `docs/calibration_protocol.md`; `configs/experiment/prep/exp_benchmark_*` | Local only | Next required step before any baseline execution | P1 |
...
5. `B0`: write and freeze matched-budget benchmark protocol.
6. `B1` + `B2`: run minimal baseline comparison package only after B0 is frozen.
