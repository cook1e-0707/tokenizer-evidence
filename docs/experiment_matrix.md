...
| B3 | External Scalable/Perinucleus baseline | official-code Qwen-adapted Scalable/Perinucleus final is standing: `48/48` valid successes, `0` method failures, `0` pending, candidate utility sanity passed; legacy adapted `baseline_perinucleus` artifacts remain excluded diagnostics | manuscript table integration with explicit adaptation label; no reruns unless claim wording changes | `results/tables/baseline_perinucleus_official_qwen_final.csv`; `results/processed/paper_stats/baseline_perinucleus_official_qwen_final_summary.json`; `results/tables/baseline_paper_registry.csv`; `results/processed/paper_stats/baseline_paper_registry_summary.json`; `docs/baseline_paper_registry.md` | Local review only | Report as `Qwen-adapted official Scalable/Perinucleus baseline`, not as unmodified full fine-tune reproduction; do not use `baseline_perinucleus.csv` for Scalable claims | P0 |
