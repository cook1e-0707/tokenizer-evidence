...
| T1 | Theorem 1 / tokenizer-contextual alignment package (`Section 4`) | `theorem1_qwen7b/contextual_exact` passed on Chimera; the `sequence_proxy` control arm is now repaired locally so it uses `scaffolded_compiled_completion`, carries an explicit `Payload label` in the scaffold prompt, and evaluates through `canonical_render` over the same compiled payload/codebook target | one clean Chimera rerun of `sequence_proxy`, then a paper-facing T1 comparison table | `configs/experiment/prep/exp_train__qwen2_5_7b__t1_*.yaml`; `configs/experiment/prep/exp_eval__qwen2_5_7b__t1_*.yaml`; `docs/t1_chimera_package.md`; future `results/tables/t1_contextual_alignment.*` | Local repair complete; Chimera rerun next | Same `Qwen/Qwen2.5-7B-Instruct`; same compiled codebook; rerun only `sequence_proxy` | P1 |
...
4. `T1`: rerun only the repaired `sequence_proxy` control arm and then consolidate the pair; `T2`: consolidate the accepted `T2-r1` theorem result into paper-facing artifacts; no new Chimera sweep unless the paper claim changes.
