...
| G3a | Block-count scale package | package is prepared locally for fixed `4 x 4` compiled codebook with `block_count=1/2/4`; `B2` reuses standing evidence | Chimera execution for new `B1` and `B4` cells: `24 train + 24 eval` | `configs/experiment/scale/exp_train__qwen2_5_7b__g3a_block_scale_v1.yaml`; `configs/experiment/scale/exp_eval__qwen2_5_7b__g3a_block_scale_v1.yaml`; `results/tables/g3a_block_scale.csv`; `results/tables/g3a_block_scale.tex`; `results/processed/paper_stats/g3a_summary.json`; `results/processed/paper_stats/g3a_run_inclusion_list.json` | Local prepare, Chimera run | Keep Qwen7B, the compiled codebook, fields, prompt family, and train payload labels fixed; vary only `block_count` in this package; write new runs to `EXP_SCRATCH` | P1 |
...
Current split:
- `G3a` varies only `block_count: 1 -> 2 -> 4` under the current single-member `4 x 4` compiled codebook.
- `G3b` is reserved for `members_per_bucket: 1 -> 2 -> 4` at fixed `block_count = 2`; do not run it before G3a lands.
...
5. `G3a` -> `G4`: run the remaining publication-scale packages on Qwen7B while keeping `G1` and `G2` frozen.
