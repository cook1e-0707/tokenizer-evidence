...
- Full FAR/null calibration, wrong-payload claim accept rate, and binary clean ownership detection are separate quantities.
- The current wrong-payload result is a structured claim-acceptance subset, not full FAR and not a general failure of Perinucleus as a binary fingerprint detector.
...
| C07 FAR-calibrated ownership decision for main comparison | no | B0 calibration exists for internal fixed/uniform baselines at target FAR `0.01`; matched runner now computes only an artifact-backed wrong-payload claim accept-rate subset | Protocol exists, full FAR/null artifact missing | Full null calibration for ours and Perinucleus at `M=1/3/5/10`, including base Qwen, wrong-owner, non-owner probes, and organic prompts |
| C08 Better utility preservation than Perinucleus | no | TinyBench sanity exists for both: ours adapter mean `0.6080` vs base `0.6035`; Perinucleus adapter `0.6192` vs base `0.6035`; Perinucleus is higher by about `0.0111` absolute | Can state both pass TinyBench sanity; do not claim ours has better utility | Broader OpenLLM-style utility, confidence intervals, and paired task-level comparison |
...
| C16 Wrong-payload structured claim rejection subset | yes | Artifact-backed matched runner: ours wrong-payload claim accept rate `0.0` for `M=1/3/5/10`; original Perinucleus wrong-payload claim accept rate `1.0` for `M=1/3/5/10` | State as a preliminary claim-binding subset. Original Perinucleus is a binary fingerprint detector, not a structured payload verifier | Full FAR/null calibration, wrong-owner checks, and a payload-adapted Perinucleus baseline |
...
- The wrong-payload claim accept-rate subset may be used to motivate structured payload binding, but only with the caveat that it is not full FAR and original Perinucleus is a binary detector.
...
- Full matched FAR/null calibration remains appendix/protocol-only until base-Qwen, wrong-owner, non-owner-probe, and organic-prompt null sets are generated.
