...
from scripts.natural_evidence_v2.build_r4_positive_selectivity_repair_package import (
    DEFAULT_CONFIG,
    build_package,
)


def test_build_selectivity_repair_package_static_validation(tmp_path: Path) -> None:
    output_dir = tmp_path / "selectivity_repair"

    summary = build_package(DEFAULT_CONFIG, output_dir)

    assert summary["status"] == "PASS_SELECTIVITY_REPAIR_PACKAGE_STATIC_VALIDATION_NO_COMPUTE"
    assert summary["event_window_rows"] >= 96
    assert summary["self_cue_rows"] == 0
    assert summary["max_surface_family_fraction"] <= 0.25
    assert summary["toy_positive_accept"] is True
    assert summary["generic_raw_task_accept"] is False
    ...
    assert summary["model_scoring_allowed"] is False

    contract = json.loads((output_dir / "contract.json").read_text(encoding="utf-8"))
    assert contract["contract_id"] == "r4_positive_selectivity_repair_v1"
    ...
    assert all(row["verb_lemmas"][0] != row["cue_lemmas"][0] for row in bank)
