...
def test_attack_script_consumes_generated_text_and_detects_delimiter_scrub(tmp_path: Path) -> None:
    repo_root = discover_repo_root(Path(__file__).parent)
    output_root = tmp_path / "results"
    frozen_catalog_path = _write_frozen_catalog(tmp_path / "carrier_catalog_freeze_v1.yaml")
    layout = BucketLayout.from_dict(yaml.safe_load(frozen_catalog_path.read_text(encoding="utf-8")))
    ...
    payload["attack"]["name"] = "delimiter_scrub"
    payload["attack"]["mode"] = "delimiter_scrub"
    payload["attack"]["strength"] = 1.0
