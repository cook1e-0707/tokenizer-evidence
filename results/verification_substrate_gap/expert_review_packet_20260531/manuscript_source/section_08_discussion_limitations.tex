...
codeword recovery, but a nonzero accept region.  We do not claim that these
...
not be described as public text-only verification success, natural evidence
...
current artifact-level result does not claim that every rewritten example is
semantically preserved, human-natural, or indistinguishable from ordinary
answers.  Adding a naturalness or semantic-preservation filter could strengthen
the attack, but it is not required to show that a predicate with zero codeword
recovery and source-mismatch accepts is not a deterministic verifier.
...
\paragraph{Unclaimed axes.}
The current artifacts do not claim sanitizer robustness, robustness to semantic
...
ownership resolution, rewritten-text ownership resolution, or non-cooperative
