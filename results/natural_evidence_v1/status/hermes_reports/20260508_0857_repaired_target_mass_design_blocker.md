...
```text
results/natural_evidence_v1/status/hermes_reports/20260508_0857_scheduled_tick_notification.json
...
The requested action is not safe or unambiguous for this phase. The current
Hermes report permits only a new explicit artifact-only negative-diagnosis or
root-cause plan, or user/expert review. A repeated repaired teacher-forced
target-mass design is not such a plan.

The same repaired design over the primary branch-aware candidates already
exists:
