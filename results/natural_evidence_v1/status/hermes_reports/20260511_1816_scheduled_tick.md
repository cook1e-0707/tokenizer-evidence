...
JOBID PARTITION     NAME     USER ST       TIME  NODES NODELIST(REASON)
            853110   DGXA100 nat-ev-v guanjie.  R      17:10      1 chimera12
```
...
notification_json:
`results/natural_evidence_v1/status/hermes_reports/20260511_1816_scheduled_tick_notification.json`

state_changing_action:
Telegram/email notification sent before launching one background Codex worker.

next_allowed_action:
Artifact-only next step: re-review 852426 replay compatibility or explicitly supersede it under the repaired R3.2 prompt split contract. Do not submit another R3.2 Slurm job until replay compatibility is re-reviewed or superseded, allowlist safety is rechecked, and a new single-job submission route is recorded.
